...
/*
* 迭代器允许枚举Module中的成员，对module使用深度优先遍历，Policy模板参数
* 确定目标的哪些slots应该被包含
* 例如，在迭代parameters时，返回的是parameter tensors，跳过modules、buffers
* 和attributes
* 关于Policy object的API注释见ModulePolicy
*/
...
/*
* 该类型表示一个列表，该列表包含了module中的parameters、attributes和submodules。
* 它是抽象的，因为它不是直接存储在std::vectors，而是存储在module的IValue对象中
*/
template <typename Policy>
struct slot_list_impl {
...
/*
* 获取一个Policy对象，并返回slot
* 以及slot的完全限定名，这用于named_variants，比如named_parameters()
*/
template <typename Policy>
struct NamedPolicy {
